...
exports.createUserProfile = onCall(
  ...
  async (request) => {
    // รับ uid จาก client โดยตรง (phone signOut แล้ว token หมด)
    const uid      = (request.auth && request.auth.uid) || (request.data && request.data.uid);
    const userData = request.data.userData;

    if (!uid) {
      throw new HttpsError('unauthenticated', 'ไม่พบ uid กรุณาลองใหม่');
    }
